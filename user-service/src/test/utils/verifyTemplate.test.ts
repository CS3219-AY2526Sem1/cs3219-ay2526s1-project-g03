...
describe('utils/verifyTemplate', () => {
  describe('getVerifyEmail', () => {
    it('should provide verify email template correctly', async () => {
      const url = 'totallyValidURL';
      const result = await getVerifyEmail(url);

      const EXPECTED_SUBJECT = 'Confirm your PeerPrep account';
      const EXPECTED_TEXT = `Click on the link to verify your email address: ${url}`;
      const EXPECTED_HTML = `
    ...
    `;
      ...
      expect(result.subject).toBe(EXPECTED_SUBJECT);
  ...
  describe('getPasswordReset', () => {
    it('should provide password reset email template correctly', async () => {
      const url = 'totallyValidURL';
      const result = await getPasswordReset(url);

      const EXPECTED_SUBJECT = 'Reset your PeerPrep password';
      const EXPECTED_TEXT = `Click on the link to reset your password: ${url}`;
      const EXPECTED_HTML = `
      <div style="font-family: Arial, sans-serif; background-color: #f7f9fc; padding: 20px; text-align: center;">
          <div style="margin-bottom: 20px;">
              <span style="font-size: 24px; font-weight: bold; color: #3b82f6;">
