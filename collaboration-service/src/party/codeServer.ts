    ...
    connection.addEventListener('close', async () => {
      ...
      setTimeout(async () => {
        ...
        if (userCount === 0) {
          ...
          console.log(`[${room.id}] All users disconnected, deleting room...`);
          try {
            const {error} = await deleteRoom(room.id);
            if (error) {
              console.error(`[${room.id}] Failed to delete room:`, error);
            } else {
              console.log(`[${room.id}] Room deleted successfully`);
            }
          } catch (err) {
            console.error(`[${room.id}] Error deleting room:`, err);
          }
        }
